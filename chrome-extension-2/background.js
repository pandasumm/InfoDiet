...
var file_exts_not_to_be_categorized = [
    ".pdf",
    ".png",
    ".jpg",
    ".jpeg",
...
// chrome.alarms.create("sync_list", {
//     delayInMinutes: 1,
//     periodInMinutes: 15
// });
...
chrome.storage.local.get([
    ...
    if (result.user_profile_to_upload == undefined) {
        user_profile_to_upload = []
        chrome.storage.local.set({'user_profile_to_upload': user_profile_to_upload});
    } else {
    ...
    if (result.per_url_policy == undefined) {
        per_url_policy = {};
        chrome.storage.local.set({'per_url_policy': per_url_policy});
    } else {
    ...
    if (result.list_topcats_to_block_network_conns == undefined) {
        list_topcats_to_block_network_conns = []
        chrome.storage.local.set({'list_topcats_to_block_network_conns': list_topcats_to_block_network_conns});
    } else {
    ...
    if (result.cache_url_to_cat == undefined) {
        cache_url_to_cat = {}
        chrome.storage.local.set({'cache_url_to_cat': cache_url_to_cat});
    } else {
    ...
    if (result.opt_out == undefined) {
        opt_out = false
        chrome.storage.local.set({'opt_out': opt_out});
    } else {
...
function submit_data_interval() {
    ...
    var main_data = JSON.stringify({user_profile_to_upload: user_profile_to_upload, uid: uid, ct: ct});
...
function submit_data(type, list) {
    ...
    if (type == 'list_topcats_to_block_network_conns') {
        var main_data = JSON.stringify({list_topcats_to_block_network_conns: list, uid: uid, ct: ct});
    } else {
        var main_data = JSON.stringify({per_url_policy: list, uid: uid, ct: ct});
    }
    ...
    if (current_type == 'main_frame') {
        ...
        if (current_domain != tabs_info[current_tabId].domain) {
            ...
            if (trackers_decision[current_domain].length > 2) {
                if (current_domain != tabs_info[current_tabId].domain && (allowed_thirdparty_domains["all_domains"].indexOf(current_domain) == -1 || (allowed_thirdparty_domains[tabs_info[current_tabId].domain] && allowed_thirdparty_domains[tabs_info[current_tabId].domain].indexOf(current_domain) == -1))) {
                ...
                if ((tab_url_hash in per_url_policy) && per_url_policy[tab_url_hash] == 'conns_allow') {
                    ...
                    if (allowed_thirdparty_domains["all_domains"].indexOf(current_domain) > -1 || (allowed_thirdparty_domains[tabs_info[current_tabId].domain] && allowed_thirdparty_domains[tabs_info[current_tabId].domain].indexOf(current_domain) > -1)) {
                        return;
                    }
    ...
    if (tabStatus == 'complete') {
        ...
        function doCollectiFramesSrc(info) {
            ...
            if (counter_for_received_response[tabId] == counter_for_iframe_response_tracking[tabId]) {
                ...
                var info_page = {
                    url: urlhash,
                    topcatlist: tabs_info[tabId].topcatList,
                    iframes_src: list_of_iframes_src,
                    trackers: tabs_info[tabId].trackers,
                    ts: Date.now()
                };
